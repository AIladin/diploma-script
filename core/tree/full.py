...
class FullBinaryTreeBuilder:
    def __init__(
        self,
        node_factory: node.NodeFactory,
        n_levels: int,
        root_node: Optional[node.Node] = None,
    ):
    ...
    def build_full_tree(
        ...
        while required_nodes > 0:
            current_node: node.Node = queue.pop()

            child_1 = self.node_factory.create_node(current_node, node.NodeType.OMEGA_1)
            queue.appendleft(child_1)
            pbar.update(1)

            child_2 = self.node_factory.create_node(current_node, node.NodeType.OMEGA_2)
